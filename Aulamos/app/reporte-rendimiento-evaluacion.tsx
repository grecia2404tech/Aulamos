    ...
    async (mostrarCarga = true) => {
      try {
        ...
        const respuesta = await fetch(
          `${API_URL}/docente/reportes/rendimiento-evaluacion?${query.toString()}`,
          {
            headers: {
        ...
        if (!respuesta.ok) {
          throw new Error(
            resultado.mensaje ||
              'No se pudo cargar el rendimiento de las evaluaciones.'
          );
        }
        ...
        const nuevosDatos = Array.isArray(
          ...
          ? resultado.evaluaciones.map(
              (item: Partial<Evaluacion>) => ({
                id_evaluacion: Number(
                  item.id_evaluacion ?? 0
                ),
                titulo:
                  item.titulo ?? 'Evaluación sin título',
        ...
        setEvaluaciones(nuevosDatos);
        anunciar(
          `Se cargaron ${nuevosDatos.length} evaluaciones.`
        );
      } catch (error) {
        console.error(
          'Error al cargar rendimiento por evaluación:',
          error
        );
  ...
  const resumen = useMemo(() => {
    ...
    return {
      ...
      aprobados:
        evaluaciones.reduce(
          (suma, item) =>
            suma + item.porcentaje_aprobados,
          0
        ) / total,
      participacion:
        evaluaciones.reduce(
          (suma, item) =>
            suma + item.porcentaje_participacion,
          0
        ) / total,
    };
  }, [evaluaciones]);
        ...
        {cargando ? (
          ...
          <>
            <View style={styles.filaResumen}>
              <TarjetaResumen
                titulo="Promedio"
                valor={resumen.promedio.toFixed(1)}
                sufijo="/10"
            ...
            {evaluaciones.length === 0 ? (
              ...
              evaluaciones.map((evaluacion) => (
                <View
                  ...
                  accessible
                  accessibilityLabel={`${evaluacion.titulo}. Promedio ${evaluacion.promedio.toFixed(
                    1
                  )} de 10. ${Math.round(
                    evaluacion.porcentaje_aprobados
                  )} por ciento de aprobados.`}
                >
                  <View style={styles.cabeceraTarjeta}>
                    <View
                      style={[
                        styles.iconoEvaluacion,
                        {
                          backgroundColor:
                  ...
                  >
                    <Metrica
                      etiqueta="Promedio"
                      valor={`${evaluacion.promedio.toFixed(
                        1
                      )}/10`}
                      colores={colores}
                      escalaTexto={escalaTexto}
                    ...
                    <Metrica
                      etiqueta="Entregas"
                      valor={`${evaluacion.total_entregas}/${evaluacion.total_alumnos}`}
                      colores={colores}
                      escalaTexto={escalaTexto}
                    />
                  </View>
